# Hi, I'm Mubasshir

I'm a student who enjoys building things with code. Most of my projects start with a random idea that I want to see if I can turn into something that actually works.

Right now, I'm mainly interested in web development, programming languages, and experimenting with new ideas.

## What I'm working on

- Building creative web projects
- Learning JavaScript and improving my web development skills
- Experimenting with programming languages
- Making things that are unnecessarily complicated for no reason

## Languages I use

HTML, CSS, JavaScript, Python, C++, Typescript and Lua.

## Other interests

Technology, coding, chemistry, and physics.

## Find me online

- [Discord](https://discord.gg/NotMubasshir)
- [Instagram](https://instagram.com/notmubasshirx)
- [Email](mailto:NotMubasshir.proton.me)

## GitHub stats

![GitHub Stats](https://github-readme-stats.shion.dev/api?username=NotMubasshir&theme=gruvbox&hide_border=false&include_all_commits=true&count_private=true)

![GitHub Streak](https://streak-stats.demolab.com/?user=NotMubasshir&theme=gruvbox&hide_border=false)

![Top Languages](https://github-readme-stats.shion.dev/api/top-langs/?username=NotMubasshir&theme=gruvbox&hide_border=false&include_all_commits=true&count_private=true&layout=compact)

---

I build things to learn, improve, and occasionally make something that is unnecessarily complicated for no reason.
